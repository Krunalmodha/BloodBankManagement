	...
	return (
		<>
			...
			<div className="bg-white p-10 m-10 -mt-20 rounded-rsm">
				...
				{/* Enable CardLineChart analytics chart */}
				<div className="w-full xl:w-8/12 mb-12 xl:mb-0 px-4">
					<CardLineChart />
				</div>

				{/* Recent Activity Section */}
				<div className="bg-white p-6 m-10 rounded-rsm">
					<h2 className="text-dark_red font-semibold mb-4 text-[28px]">Recent Activity</h2>
					<ul>
						{/* Replace with real recent activity data if available */}
						{data.slice(0, 5).map((item, idx) => (
							<li key={item.id || idx} className="mb-2 text-gray-700">
								<span className="font-bold">{item.name}</span> - {item.source || 'Action'} on {item.date || 'N/A'}
							</li>
						))}
						{data.length === 0 && <li className="text-gray-400">No recent activity.</li>}
					</ul>
				</div>
